...
while(<FH>){
	chomp;
	my @splited_line = &parse_line(",", undef, $_);		
	#XXXXX2=first item of clinical notes of prenatal checkup
	#XXXXX3=second item of clinical notes of prenatal checkup (if available)
	#XXXXX4=first clinical notes at delivery
	#XXXXX5=second clinical notes at delivery (if availbale)
	#XXXXX6=third clinical notes at delivery (if availbale)
	if(($splited_line[1] eq "XXXXX2") or ($splited_line[1] eq "XXXXX3") or ($splited_line[1] eq "XXXXX4")or ($splited_line[1] eq "XXXXX5") or ($splited_line[1] eq "XXXXX6")){
		...
		foreach my $each_item(@splited){	
			if(($each_item =~ /肝機能障害/) and ($each_item !~ /ウイルス感染/) and ($each_item !~ /中学生くらいから/) and ($each_item !~ /はないが/)){
