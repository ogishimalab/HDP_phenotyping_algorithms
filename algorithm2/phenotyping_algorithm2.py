...
def checker_core(row, col_num, rep):
  ...
  for num in col_num:
    ...
    if (data == "－"):
      data = "-"
    if ((data != "") and (data != "@") and (int(data) != rep)):
      flag = 1
      return flag
  return flag
...
def hp_checker(row, high_bp, low_bp, id):
  check = 0
  ...
  for y in range(0, len(low_bp)):
    ...
    if id == "338045":
      print("BP")
      print(low_data_point)
      print(row[low_data_point])
...
def get_fgr_flag(cid, fgr_dict, fgr_week_dict, bp_week):
  fgr_flag = 0
  if ((cid in fgr_dict) and (int(fgr_dict[cid]) == 1)):
    fgr_week = fgr_week_dict[cid]
    fgr_week = float(fgr_week)
    bp_week = float(bp_week)	
    if bp_week <= fgr_week:
      fgr_flag = 1
  return fgr_flag 
...
def phenotyping(file, classed, col_num_dict,ht_dict, bd_dict, fgr_dict, fgr_week_dict):
  ...
  for row in f:
    ...
    if count == 1:
      ...
      if bp_null_flag == 1:
        print (str(id) + ": bp null")
      else:
        if id in ht_dict:
          pu_week_ret = get_first_PIH_pu(pu, pu_week, row)
          if pu_week_ret == 100:
            #if patients have systematic disorder, classify patients into SuPE#
            if id in bd_dict:
              ga.append(id)
              sys_dis_week = bd_dict[id]
              if (float(sys_dis_week) < 34):
          ...
          check = hp_checker(row, high_bp, row_bp, id)
          if check == 0:
            ...
            if (bp_week >= 20):
              if pu_week_ret == 100:
                #if patients have systematic disorder after observation of high blood pressure, classify patients into PE#
                sysdis_flag = get_sys_dis(id, bd_dict, bp_week)
                if sysdis_flag == 1:
                  #jadge onset point#
                  sys_dis_week = bd_dict[id]
                  ga.append(id)
                  ...
                  fgr_flag = get_fgr_flag(cid, fgr_dict, fgr_week_dict, bp_week)
                  if fgr_flag == 1:
                    fgr_week = fgr_week_dict[cid]
                    ga.append(id)					
                    if (float(fgr_week) < 34):
              ...
              if pu_week_ret == 100:
                #if patients have systematic disorder after observation of high blood pressure, classify patients into SuPE#
                sysdis_flag = get_sys_dis(id, bd_dict, bp_week)
                if sysdis_flag == 1:
                  ga.append(id)				
                  #jadge onset point#
...
def get_fgr_dict(file):
  ...
  for row in f:
    ...
    fgr_flag = row[1]
    fgr_week = row[2]
    fgr_dict[id] = fgr_flag
    fgr_week_dict[id] = fgr_week
  return (fgr_dict,fgr_week_dict)
	
file = "input_file.csv"
col_num_dict = get_colomn_num(file)
out = "converdate_date.csv"
classed = "phenotyped7.csv"
ht = "hts.csv"
bd = "body_dist.withGA.csv"
FGR_LIST = "fgr_list.csv"
ht_dict = get_ht_ids(ht)
bd_dict = get_bd_dict(bd)
fgr_dict, fgr_week = get_fgr_dict(FGR_LIST)
convert_date(file, out, col_num_dict)
phenotyping(out, classed, col_num_dict, ht_dict, bd_dict, fgr_dict, fgr_week)